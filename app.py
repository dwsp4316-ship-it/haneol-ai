...
def get_loading_message(text):
    try:
        ...
        loading_messages = {
            ...
            "ja": "🧭 HANEOL가`app.py` 내용 입력 후, 맨 아래로 스크롤을 내려 초록색 **`Commit changes...`** 버튼을 클릭하여 저장합니다.

---

### 2. `requirements.txt` 파일 만들기

`app.py` 저장이 완료되면 저장소 메인 페이지로 돌아오게 됩니다.

1. 파일 목록 우측 상단의 **`Add file`** 버튼을 누른 뒤 **`Create new file`**을 클릭합니다.
2. 파일 이름 칸(`Name your file...`)에 **`requirements.txt`** 라고 입력합니다.
3. 텍스트 입력 창에 아래 3줄을 복사해 넣습니다:
